...
def compare_arguments_mock(args1, kwargs1, args2, kwargs2, tmpdir):
    ...
    assert_frame_equal(
        args1[0].df, args2[0].df
    ), "Expected .df member of ASReviewData instances to be equal."
    assert (
        kwargs1.keys() == kwargs2.keys()
    ), "Expected SimulateReview to be called with the same keyword arguments."
    for (k1, v1), (k2, v2) in zip(kwargs1.items(), kwargs2.items()):
        assert type(v1) == type(
            v2
        ), f"Expected type of SimulateReview's keyword argument '{k1}' to be equal."
        if isinstance(v1, ASReviewProject):
            ...
            assert (
                v1 == v2
            ), f"Expected SimulateReview's keyword argument '{k1}' to be equal."
